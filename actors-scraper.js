...
const url4 =
  "https://www.cinemaofisrael.co.il/%d7%99%d7%94%d7%95%d7%93%d7%94-%d7%91%d7%a8%d7%a7%d7%9f/";

export const extractActorsNameByUrl = async url => {
  let html;
  let count = 0;
  let maxTries = 3;
  while (true) {
    try {
      html = await requestPromise(url);
      break;
    } catch (err) {
      if (++count == maxTries) return {};
    }
  }
  const name = await extractActorsName(html);
  return { שם: name };
};
  ...
  actor_data.forEach(element => {
    ...
    for (var key in element) {
      ...
      if (value == "משחק") {
        ...
        into_awards = 1;
      else if (value == "ביוגראפיה") into_biography = 1;

      if (value.indexOf(",") != -1) {
        let splited_val = value.split(", ");

        if (into_actors && i++ != 1) {
          //ignore double record in first movie
  ...
  clean_obj.forEach(element => {
    ...
    for (var key in element) {
      ...
      if (detail != "ביוגראפיה" && detail != "משחק") {
        ...
        if (end_idx == -1) end_idx = value.indexOf("("); //no space befor '('
        if (end_idx == -1) end_idx = value.length;
